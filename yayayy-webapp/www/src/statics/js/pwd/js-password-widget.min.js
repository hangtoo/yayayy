/*!CSII.WebPassword - v1.0.1  - 2016-05-03
...
function rng_seed_int(a) {
  rng_pool[rng_pptr++] ^= 255 & a, rng_pool[rng_pptr++] ^= 255 & a >> 8, rng_pool[rng_pptr++] ^= 255 & a >> 16, rng_pool[rng_pptr++] ^= 255 & a >> 24, rng_pptr >= rng_psize && (rng_pptr -= rng_psize)
}
...
Montgomery.prototype.reduce = montReduce, Montgomery.prototype.mulTo = montMulTo, Montgomery.prototype.sqrTo = montSqrTo,
...
BigInteger.prototype.multiplyTo = bnpMultiplyTo, BigInteger.prototype.squareTo = bnpSquareTo,
BigInteger.prototype.divRemTo = bnpDivRemTo, BigInteger.prototype.invDigit = bnpInvDigit, BigInteger.prototype.isEven = bnpIsEven,
BigInteger.prototype.exp = bnpExp, BigInteger.prototype.toString = bnToString, BigInteger.prototype.negate = bnNegate,
BigInteger.prototype.abs = bnAbs, BigInteger.prototype.compareTo = bnCompareTo, BigInteger.prototype.bitLength = bnBitLength,
BigInteger.prototype.mod = bnMod, BigInteger.prototype.modPowInt = bnModPowInt, BigInteger.ZERO = nbv(0),
BigInteger.ONE = nbv(1), Arcfour.prototype.init = ARC4init, Arcfour.prototype.next = ARC4next;
var rng_psize = 256,
...
RSAKey.prototype.encrypt = RSAEncrypt,
  ...
  function(a, b) {
    "use strict";
    if (b) {
      var c = b.module("CSII.WebPassword", []);
      c.directive("vPasswordWidget", ["$log", function() {
        return {
          restrict: "AE",
          link: function(a, c, d) {
            var e = $.extend({}, b.fromJson(d.options || {}));
            e.UseCache ? $(c).PasswordWidget(e) : ($("#_PassWordWidget" + e.KeyType).empty(), $(c).PasswordWidget(e))
          }
        }
      }])
    }
  ...
  export default {
    ...
    init(a, b) {
      "use strict";
      b.fn.$getCiphertext = function() {
        var a = $(this);
        return a.attr("data-ciphertext")
      },
      b.fn.$getPasswordLength = function() {
        var a = $(this);
        return a.attr("data-passwordlength")
      };

    },

    PasswordWidget (a,b,e) {
      ...
      $.extend(c, e || {});
      var f, g, h = $("#vPasswordWidget"),
        i = h.attr("modulus-hex"),
        j = h.attr("isShowKeyboard"),
        k = "10001",
        l = new RSAKey,
        m = h.html(),
        n = h.attr("maxlength") ? parseInt(h.attr("maxlength")) : 10,
        o = c.keyDownStyle || {
          "background-color": "#808080",
          opacity: "0.5"
        },
        p = h.attr("password-widget"),
        q = document.body.clientWidth,
        r = [];
        //console.log(h);

      q > 570 && (q = 570), a.orientation && 0 != a.orientation && (q = .6 * document.body.clientWidth), i ? l.setPublic(i, k) : alert("please set modulus_hex!"), h.css({
        color: "#999999",
      ...
      h.html("正在加载。。"),
      $("#_PassWordWidget" + c.KeyType).length ? f = $("#_PassWordWidget" + c.KeyType) : (f = $('<div id="_PassWordWidget' + c.KeyType + '" onselectstart="return false;" style="display:none;position:fixed;left:0px;bottom: 0px;z-index: 9003;width: 100%; height:' + 333 * q / 576 + 'px;background-color:#D9DADF;-moz-user-select: none;-webkit-user-select: none;-ms-user-select: none;-khtml-user-select: none; user-select: none;"></div>'),
      $("body").append(f).append('<div id="_PassWordShadow" style="display: none;position: absolute;top: 0;left: 0;width: 100%;height: 100%;overflow: hidden;z-index: 8999;opacity: 0;"></div>'));
      var s = function() {
          $("#_PassWordWidget" + c.KeyType + ",_PassWordShadow").remove(), h.html(m)
        },
        t = function(a) {
          $("#_PassWordShadow").hide(), f.hide().find("#LowercaseDiv,#UppercaseDiv,#NumberDiv").hide(),
          a.attr("data-ciphertext", l.encrypt(r.join(","))),
          a.attr("data-passwordlength", r.length), 0 === r.length && (a.css("color", "#999999"), a.html(m))
        },
        u = function(a) {
          ...
          f.find("#LowercaseDiv>div,#UppercaseDiv>div,#NumberDiv>div").bind("touchstart", function() {
            //alert("touchstart");
            $(this).css(o);
          }).unbind("click").bind("click", function() {
            //alert("touchend");
            var b = $(this);
            if (b.siblings("div").css({
                "background-color": "#FFFFFF",
                opacity: "0"
              }), b.css({
                "background-color": "#FFFFFF",
                opacity: "0"
              }), b.attr("data-keyindex"))
              r.length < n && (r.push(b.attr("data-keyIndex")), 1 === r.length ? a.html("*") : a.html(a.text() + "*"));
            else {
              var c = b.attr("data-funcindex");
              switch (c) {
                case "F1":
                  f.find("#LowercaseDiv,#UppercaseDiv").toggle();
                  break;
                case "F2":
                  r.length > 0 && a.text(a.text().substr(1)) && r.pop(), 0 == r.length && a.text(m);
                  break;
                case "F3":
                  f.find("#" + g).show(), f.find("#NumberDiv").hide();
                  break;
                case "F4":
                  f.find("#LowercaseDiv,#UppercaseDiv").hide(), f.find("#NumberDiv").show(), g = b.parent().attr("id");
                  break;
                case "F5":
                  t(a)
              }
        ...
        v = function() {
          f.children().css("left", (document.body.clientWidth - q) / 2 + "px")
        };
        d || c.UseCache && f.children().length>1 ? h.html(m) :
        (d++, axios({
          method: 'post',
          baseURL: 'http://localhost:8080',
          url:p,
          data: JSON.stringify({"bank_id":1,"channel_id":1,"width":""+q}),
          timeout: 10000,
          headers: {
        ...
        if (d--) {
            f.html('<div style="position:relative;top:0px;left:0px;width: 100%;height: 100%;z-index: 9003;">' + res.data.data.trsPassword + "</div>");
            f.css("height", f.children().children("div:first").height());
            v();
            h.html("请输入密码"), "true" == j;
            u(h);
          //if (!f.children().length) return;
          //h.css("color", "#000000"), $("#_PassWordShadow").show(), f.show().children().children("div:first").show(), u(h)
        }
      }).catch(function() {
        d--, alert("password widget load error!"), s()
      })),
      h.bind("click", function() {
        f.children().length && (h.css("color", "#000000"), $("#_PassWordShadow").show(), f.show().children().children("div:first").show(), u($(this)))
      });
      var w = "onorientationchange" in a ? "orientationchange" : "resize";
      a.addEventListener(w, function() {
